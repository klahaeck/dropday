  ...
  return <>
    ...
    {assignment && assignmentClub && <section className="dashboard-grid">
      <div className="panel next-drop">
        ...
        <div className="next-drop-copy"><div className="eyebrow-row"><Pill tone="orange"><CalendarDays size={12} /> Next drop</Pill><span className="tiny-label">{formatDateTime(assignment.scheduledFor, assignmentClub.schedule.timezone)}</span></div><h2>{assignment.playlist?.title ?? assignmentClub.currentTheme.name}</h2>{assignment.playlist ? <PlaylistDescription html={assignment.playlist.descriptionHtml} fallback={assignment.playlist.description} /> : <ThemeDescription html={assignmentClub.currentTheme.guidanceHtml} fallback={assignmentClub.currentTheme.guidance ?? ""} />}<div className="countdown"><span><strong>04</strong><small>days</small></span><span><strong>18</strong><small>hours</small></span><span><strong>32</strong><small>minutes</small></span></div><div style={{ display: "flex", alignItems: "center", gap: 10 }}><Avatar user={assignedUser ?? undefined} /><strong>{assignment.assignedUserId === profile.id ? "You are dropping" : `${assignedUser?.displayName ?? "A member"} is dropping`}</strong></div></div>
      </div>
      <aside className={`theme-card${assignmentClub.currentTheme.imageUrl ? " theme-card-has-image" : ""}`}>{assignmentClub.currentTheme.imageUrl && <Image src={assignmentClub.currentTheme.imageUrl} alt="" fill sizes="(max-width: 800px) 100vw, 33vw" unoptimized />}<span className="pill pill-green">Current theme</span><h2>{assignmentClub.currentTheme.name}</h2><ThemeDescription html={assignmentClub.currentTheme.guidanceHtml} fallback={assignmentClub.currentTheme.guidance ?? ""} /><div className="theme-card-foot"><span>{assignmentClub.name}</span><span>Theme #{assignmentClub.currentTheme.version}</span></div></aside>
    </section>}

    <div className="section-title-row"><h2>Your clubs</h2><Link href="/app/clubs">View all</Link></div>
    <div className="club-grid">{clubs.slice(0, 3).map((club) => <ClubCard key={club.id} club={club} member />)}</div>
