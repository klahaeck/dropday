...
export async function PATCH(
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "Choose the restore action." }, { status: 400 });
  }
  if (!(await consumeRateLimit(`club-backups:${profile.id}`, 20, 60))) {
    return NextResponse.json(
      { error: "Too many backup changes. Try again in a minute." },
  ...
  try {
    const backup = await restoreClubBackup({
      clubSlug: slug,
      backupId,
      actorUserId: profile.id,
    });
    ...
    return NextResponse.json({ error: "Could not restore this backup playlist." }, { status: 500 });
  }
}
